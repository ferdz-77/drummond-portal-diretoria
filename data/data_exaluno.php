...
function getChamadosFinalizadosExAluno() {
    try {
        $pdo = connectDB(getProductionDatabaseName('exaluno'));
        $stmt = $pdo->query("SELECT COUNT(*) as count FROM " . TABLE_EXALUNO . " WHERE " . COL_STATUS_EXALUNO . " IN ('Transferido', 'Finalizado', 'Fechado')");
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
